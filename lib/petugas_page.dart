    ...
    return InkWell(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Icon(icon,
                size: 22, color: isSelected ? activeColor : inactiveColor),
...
class _PetugasHomeScreenState extends State<PetugasHomeScreen> {
  ...
  final TextEditingController _kodeTransaksiController =
  ...
  Future<void> _lookupByCode() async {
    ...
    WasteDepositModel? findIn(List<WasteDepositModel> list, String target) {
      for (final d in list) {
        if (d.code.toLowerCase() == target.toLowerCase()) return d;
      }
      return null;
    }

    var found = findIn(_pendingDeposits, code);
    if (found == null) {
      try {
        final all = await WasteService.getDeposits();
        found = findIn(all, code);
      } catch (e) {
        if (mounted) {
    ...
    if (found == null) {
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Kode transaksi tidak ditemukan.')));
      return;
    }
    _showVerificationSheet(found);
  }
    ...
    showDialog(
      ...
      builder: (dlgCtx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            const Text(
                'Masukkan atau tempel (paste) kode tiket setoran nasabah:',
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            child: const Text('Cari & Verifikasi',
                style: TextStyle(
  ...
  Future<void> _openQrCameraScanner() async {
    final scannedCode = await Navigator.push<String>(
      context,
      MaterialPageRoute(builder: (context) => const QrScannerScreen()),
    );
    if (scannedCode != null && scannedCode.isNotEmpty && mounted) {
      _kodeTransaksiController.text = scannedCode;
      _lookupByCode();
    }
  }
                ...
                children: [
                  ...
                  ValueListenableBuilder<UserModel?>(
                    ...
                    builder: (context, user, _) {
                      ...
                      final id = user != null
                          ? 'PET-${user.id.toString().padLeft(4, '0')}'
                          : 'PET-0000';
                      return Container(
                  ...
                  Container(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const Text('Pindai QR Tiket User',
                            style: TextStyle(
                                fontSize: 14,
                                fontWeight: FontWeight.w800,
                                color: textDark)),
                        const SizedBox(height: 2),
                        const Text('Arahkan kamera ke QR code tiket nasabah',
                            style: TextStyle(fontSize: 11, color: textGray)),
                        const SizedBox(height: 16),
                        GestureDetector(
                          onTap: _openQrCameraScanner,
                          child: Container(
                            height: 160,
                            width: double.infinity,
                            decoration: BoxDecoration(
                        ...
                        const Center(
                            child: Text('Ketuk untuk membuka kamera',
                                style:
                            ...
                            Expanded(
                              child: SizedBox(
                                ...
                                child: ElevatedButton.icon(
                                  onPressed: _openQrCameraScanner,
                  ...
                  Row(
                    children: [
                      const Expanded(
                          child: Divider(color: cardBorderColor, thickness: 1)),
                      Padding(
                        padding: const EdgeInsets.symmetric(horizontal: 10.0),
                        child: Text('ATAU VERIFIKASI MANUAL',
                            style: TextStyle(
                                fontSize: 10,
                                fontWeight: FontWeight.w700,
                                color: textGray,
                    ...
                    child: Column(
                      ...
                      children: [
                        Row(
                          ...
                          children: [
                            const Text('Kode Transaksi Setoran',
                                style: TextStyle(
                                    fontSize: 12,
                                    fontWeight: FontWeight.w800,
                                    color: textDark)),
                            Text('Contoh: ECP-2026-001',
                                style:
                                    TextStyle(fontSize: 10, color: textGray)),
                          ],
                        ),
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      GestureDetector(
                        ...
                        child: const Text('Semua Antrean >',
                            style: TextStyle(
                                fontSize: 11,
      ...
      child: Column(
        children: [
          Icon(Icons.inbox_outlined,
              size: 40, color: textGray.withValues(alpha: 0.5)),
          const SizedBox(height: 8),
          const Text('Tidak ada antrean pending',
              style: TextStyle(
                  fontSize: 13, fontWeight: FontWeight.w700, color: textGray)),
          const SizedBox(height: 4),
          Text('Tarik ke bawah untuk refresh',
              style: TextStyle(
                  fontSize: 11, color: textGray.withValues(alpha: 0.7))),
        ],
      ),
    );
  }
                ...
                children: [
                  ...
                  const Text('Riwayat Verifikasi',
                      style: TextStyle(
                          ...
                          letterSpacing: -0.5)),
                  const SizedBox(height: 4),
                  const Text('Daftar seluruh setoran masuk di Petugas',
                      style: TextStyle(fontSize: 12, color: textGray)),
                  const SizedBox(height: 16),
                  Row(
                    children: [
                  ...
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      const Text('SETORAN MASUK TERBARU',
                          style: TextStyle(
                              fontSize: 10,
                              fontWeight: FontWeight.w800,
                              color: textGray,
                              letterSpacing: 0.5)),
                      Text('Total: ${filtered.length} Tiket',
                          style: const TextStyle(
                              fontSize: 10,
                              fontWeight: FontWeight.w600,
                              color: textGray)),
          ...
          children: [
            ...
            Row(
              children: [
                ...
                const SizedBox(width: 6),
                Text('•  ${_formatDate(deposit.createdAt)}',
                    ...
                    children: [
                      ...
                      Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 2),
                          Text(
                              'Est: ${deposit.weightKg.toStringAsFixed(1)} kg (~${deposit.estimatedPoints} Poin)',
                              style: const TextStyle(
              ...
              children: [
                Row(
                  children: const [
                    ...
                    SizedBox(width: 4),
                    Text('Ketuk untuk proses verifikasi timbangan',
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
              child: Text(
                  'Gagal memuat riwayat. Tarik ke bawah untuk coba lagi.',
                  style: TextStyle(fontSize: 12, color: Colors.red.shade700))),
        ],
      ),
    );
  ...
  Widget _buildEmptyState() {
    return Container(
      ...
      child: Column(
        children: [
          Icon(Icons.inbox_outlined,
              size: 48, color: textGray.withValues(alpha: 0.5)),
          const SizedBox(height: 10),
          const Text('Belum ada riwayat setoran',
              style: TextStyle(
                  fontSize: 14, fontWeight: FontWeight.w700, color: textGray)),
          const SizedBox(height: 4),
          Text('Tarik ke bawah untuk refresh',
              style: TextStyle(
                  fontSize: 11, color: textGray.withValues(alpha: 0.7))),
              ...
              children: [
                      ...
                      Row(
                        children: [
                          ...
                          const Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text('Keamanan Akun Petugas',
                                  style: TextStyle(
                                      fontSize: 13,
                                      fontWeight: FontWeight.w800,
                                      color: textDark)),
                              Text('Ganti kata sandi secara berkala',
                                  style:
                                      TextStyle(fontSize: 10, color: textGray)),
                            ],
                          ),
                ...
                Container(
                  ...
                  child: TextButton.icon(
                    ...
                    label: const Text('Keluar dari Akun Petugas',
                        style: TextStyle(
                            fontSize: 12,
                            fontWeight: FontWeight.w700,
